...
modifications <- paste0("gps_by_zip_year_", n_attempts, "attempts_boot_test") # to be used in names of output files, to record how you're tuning the models

# get m out of n bootstrap sample
boot_sample_number <- 1 # to do: replace with SLURM_ARRAY_TASK_ID

boot_zips <- read.csv(paste0(dir_code, "analysis/pm_boot_zips.csv"))
boot_zips <- boot_zips[, boot_sample_number]
# boot_zips_with_replace <- read.csv(paste0(dir_code, "analysis/pm_boot_zips_with_replace.csv"))
# boot_zips_with_replace <- boot_zips_with_replace[, boot_sample_number]
...
# boot_zip_year_data_with_replace <- zip_year_data[zip %in% boot_zips_with_replace]
# boot_zip_year_data_with_strata_with_replace <- zip_year_data_with_strata[zip %in% boot_zips_with_replace]
...
parametric_model_summary$p.table["w", "Estimate"]


##### Compute bootstrap estimate and SE #####
